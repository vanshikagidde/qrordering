<?php
// admin_shops.php - Manage All Shops (Super Admin)
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_status'])) {
    ...
    $new_status = trim($_POST['change_status']);

    $allowed = ['pending', 'active', 'rejected'];
    if ($shop_id > 0 && in_array($new_status, $allowed, true)) {
        ...
        if (!$stmt) {
            ...
            if ($stmt->execute()) {
                $message = "Shop status updated to " . ucfirst($new_status) . "!";
...
$query = "SELECT id, shop_name, owner_name, email, phone, status FROM shops WHERE 1=1";
$params = [];
$types = "";
...
if ($search !== '') {
    $query .= " AND (shop_name LIKE ? OR email LIKE ? OR owner_name LIKE ? OR phone LIKE ?)";
    $like = "%$search%";
...
<title>Manage Shops - RestoFlow Admin</title>
...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-toggle" id="sidebarToggle"><i class="fas fa-chevron-left"></i></div>
    ...
    <nav class="nav-menu">
        ...
        <a href="admin_shops.php" class="nav-item active">
...
<main class="main-content" id="mainContent">
    <div class="top-bar">
        <div class="welcome-message">
            <h1>Manage Shops</h1>
            <p>Approve, reject, or monitor all registered restaurants</p>
        </div>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h3><?= number_format($pending_shops) ?></h3>
        ...
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h3><?= number_format($rejected_shops) ?></h3>
                <p>Rejected Shops</p>
            </div>
        </div>
    </div>
        ...
        <form method="get" class="filter-bar">
            ...
            <div class="filter-group">
                ...
                <input type="text" name="search" class="filter-input" 
                       placeholder="Shop name, email, owner..." 
    ...
    <div class="card">
        <div class="card-header">
            <h2 class="card-title">
                ...
                All Shops (<?= number_format(count($shops)) ?>)
            </h2>
        </div>
        ...
        <?php if (empty($shops)): ?>
            <div class="empty-state">
                ...
                <p>Try adjusting your filters or no shops have been registered yet.</p>
            </div>
        <?php else: ?>
            ...
            <table>
                ...
                <?php foreach ($shops as $shop): ?>
                    <tr>
                        ...
                        <td>
                            <?= htmlspecialchars($shop['email']) ?><br>
                            <small style="color: var(--gray-600);"><?= htmlspecialchars($shop['phone'] ?: '—') ?></small>
                        </td>
                        <td>
                            ...
                            <form method="post" style="display:inline;">
                                ...
                                <?php if ($shop['status'] === 'pending'): ?>
                                    <div class="table-actions">
                                        <button type="submit" name="change_status" value="active" class="btn-action btn-approve">
                                            <i class="fas fa-check"></i> Approve
                                        </button>
                                        <button type="submit" name="change_status" value="rejected" class="btn-action btn-reject">
                                            <i class="fas fa-times"></i> Reject
                                        </button>
                                    </div>
                                <?php elseif ($shop['status'] === 'active'): ?>
                                    <button type="submit" name="change_status" value="rejected" class="btn-action btn-deactivate">
